...
bool Player::checkBombs()		//RRR Check if player can place bombs.
{
	bool returnbombs = false; 
	if(bombs != 0)
	{
		returnbombs = true;
	}
		...
		case 1:											//LEFT
			if(yStep == 0){								//checks if their is an ability to move up or down because of the offset on the other axis
				if((xStep == 0)){						//checks if the player is entering a new block 
					if(!((MP->getFieldValue(xPos - 1, yPos) == 1) || (MP->getFieldValue(xPos - 1, yPos) == 2) || (MP->getFieldValue(xPos - 1, yPos) == 5))) {		//checks if the block where the player is moving to can be entered
		...
		case 3:											//UP
			if((xStep == 0)){							//checks if their is an ability to move up or down because of the offset on the other axis
				if(yStep == 0){							//checks if the player is entering a new block 
					if(!((MP->getFieldValue(xPos, yPos - 1) == 1) || (MP->getFieldValue(xPos, yPos - 1) == 2) || (MP->getFieldValue(xPos, yPos - 1) == 5))){		//checks if the block where the player is moving to can be entered
						if(yPos > 0){
