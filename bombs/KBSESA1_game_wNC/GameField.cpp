...
#define PLAYERA		RGB(255, 0, 0)				//This is the color of player1.	
#define PLAYERB		RGB(0, 255, 0)				//This is the color of player2.
#define BOMB		RGB(0, 0, 255)				//This is the color of the bomb.		RRR
#define EXPLOSION   RGB(255, 100, 0)			//This is the color of the explosion.	RRR
  ...
  for(uint8_t y = 0; y < 9; y++)
  {
	  for(uint8_t x = 0; x < 11; x++)			//those loops are looping through the whole playing field
	  {
		  switch(mp->getFieldValue(x, y)){
			  ...
			  case 5:			//Bomb RRR
			  lcd->fillRect(leftcornerX, leftcornerY, SIZE, SIZE, BOMB);
			  break;
...
void GameField::updateGameField_bomb_nc() //RRR
{
	lcd->fillRect(pl_nc->getOldXPosPx() + OFFSETX, pl_nc->getOldYPosPx() + OFFSETY, SIZE, SIZE, FIELD);	//Draw background.
	lcd->fillRect(pl_nc->getxPosPx() + OFFSETX, pl_nc->getyPosPx() + OFFSETY, SIZE, SIZE, BOMB);		//Draw bomb.
}
