...
GameField::GameField(MI0283QT9* lcd_g, Map* mp_g, WalkingAnimation* WA_g, Player* pl_nc_g, Player* pl_ir_g){
	...
	lcd->fillScreen(BLOCK);				//resets the screen
...
void GameField::updateGameField_pl_nc(){
	/************************************************************************/
	/* Draws background before player gets drawed                           */
	/************************************************************************/
	if(pl_nc->getOldXStep()){		//checks if the player has an offset on the x axis if yes it means the block x + 1 has to be redrawn
		if(mp->getFieldValue((pl_nc->getOldXPos() + 1), pl_nc->getOldYPos()) == 0){				//checks if block = empty
			lcd->fillRect(pl_nc->getOldXPosPx() + OFFSETXSIZE, pl_nc->getOldYPosPx() + OFFSETY, SIZE, SIZE, BLACK);
		} else if(mp->getFieldValue(pl_nc->getOldXPos() + 1, pl_nc->getOldYPos()) >= 5){		//checks if block is a bomb
			lcd->fillRect(pl_nc->getOldXPosPx() + OFFSETXSIZE, pl_nc->getOldYPosPx() + OFFSETY, SIZE, SIZE, RED2);
		}
	}

	if(pl_nc->getOldYStep()){		//checks if the player has an offset on the y axis if yes it means the block y + 1 has to be redrawn
		if(mp->getFieldValue(pl_nc->getOldXPos(), (pl_nc->getOldYPos() + 1)) == 0){				//checks if block = empty
			lcd->fillRect(pl_nc->getOldXPosPx() + OFFSETX, pl_nc->getOldYPosPx() + OFFSETYSIZE, SIZE, SIZE, BLACK);
		} else if(mp->getFieldValue(pl_nc->getOldXPos(), (pl_nc->getOldYPos() + 1)) >= 5){		//checks if block is a bomb
			lcd->fillRect(pl_nc->getOldXPosPx() + OFFSETX, pl_nc->getOldYPosPx() + OFFSETYSIZE, SIZE, SIZE, RED2);
		}
	}

	if(mp->getFieldValue(pl_nc->getOldXPos(), pl_nc->getOldYPos()) == 0){					//checks if block = empty
		lcd->fillRect(pl_nc->getOldXPosPx() + OFFSETX, pl_nc->getOldYPosPx() + OFFSETY, SIZE, SIZE, BLACK);
	} else if(mp->getFieldValue(pl_nc->getOldXPos(), pl_nc->getOldYPos()) >= 5){			//checks if block is a bomb
		lcd->fillRect(pl_nc->getOldXPosPx() + OFFSETX, pl_nc->getOldYPosPx() + OFFSETY, SIZE, SIZE, RED2);
	}
	//draw new player position in state
...
void GameField::explodeBomb(uint8_t index){
	...
	for(uint8_t dir = 1; dir < 5; dir++){					//loops through each direction to destroy all blocks in that direction
		for(uint8_t explodeoffset = 1; explodeoffset <= bombs[index]->getRange(); explodeoffset++){			//checks every block from offset 1 to explode range for every direction in above for
			uint8_t explosionX;				
			uint8_t explosionY;
...
// 				Serial.println("block destroyed break");
				break;
